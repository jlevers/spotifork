 ...
 */
Spotifork.prototype.getPredictions = function(name, author, offset = 0, callback) {
    var tempThis = this;
    var spotifyApi = this.spotifyApi;
    var info = {
        matches: [],
        ...
        spotifyApi.getUserPlaylists(author, {limit: 50})
            .then(function(data) {
                ...
                async.forEachOf(data.body.items, function(playlist, index, localCallback) {
                    ...
                    if (err) {
                        callback({
                            message: 'There was an error iterating over the author\'s playlists: ' + err
                        });
                    } else {
                        callback(null, info);
                    }
                ...
                callback({
                    message: 'Error getting user\'s playlists: ' + err
                });
            });
    } else {
