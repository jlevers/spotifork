 ...
 */
function fork(playlist, owner, forker, publicity, callback) {
    var original = spotifyApi.getPlaylist(owner, playlist, {'fields': 'id, name'})
        .then(function(data) {
            spotifyApi.createPlaylist(forker, 'Fork of ' + data.body.name, { 'public': (publicity !== null ? publicity : false) })
                .then(function(forkData) {
                    copySongs(data.body.id, owner, forkData, function(err) {
                        if (err) {
                            callback({
                                message: err.message
                            });
 ...
 */
function merge(playlistIds, owners, merger, publicity, callback) {
    var playlistNames = '';
    // Create playlist to merge other playlists into
    var tmp = spotifyApi.createPlaylist(merger, 'temp', { 'public': (publicity !== null ? publicity : false) })
 ...
 */
function copySongs(originalId, owner, copy, callback) {
 ...
 *  callback with data
 */
function getPredictions(name, author, callback) {

    // Get all playlists belonging to author
    spotifyApi.getUserPlaylists(author)
        .then(function(data) {
            var display_name;
            var info = { matches: [] };
            spotifyApi.getUser(author)
