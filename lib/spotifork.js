 ...
 */
function merge(playlistIds, owners, merger, publicity = false) {
    var playlistNames = '';

    // Create playlist to merge other playlists into
    var tmp = spotifyApi.createPlaylist(merger, 'temp', {'public': publicity})
        .then(function(createData) {
            async.forEachOf(playlistIds, function(playlistId, index, callback) {
                // Get playlist name
                var tmp = spotifyApi.getPlaylist(owners[index], playlistId)
                    .then(function(getData) {
                        ...
                        copySongs(playlistId, owners[index], createData);
                        callback();
                    }, function(err) {
...
/**
 * getPrections() searches for playlists whose names begin with the user's input.
 *
 * @param String input
 *  the text that the user has typed so far (should be at least 3 chars)
 * @param Function callback
 *  callback with data
 */
function getPredictions(input, callback) {
    // Search for playlists that begin with @input 
    spotifyApi.searchPlaylists(input)
        .then(function(data) {
            async.forEachOf(data.body.playlists.items, function(playlist, index, localCallback) {
                ...
                spotifyApi.getUser(playlist.owner.id)
                    .then(function(userData) {
                        // Include user display name with each playlist
                        //if (userData.body.display_name !== null) {
                            data.body.playlists.items[index].owner.display_name = userData.body.display_name;
                            localCallback();
                       // }
                    }, function (err) {
                        console.log('Error getting name of playlist owner: ' + err);
                    });
            }, function(err) {
                if (err) {
                    console.log('There was an error iterating over playlist owners: ' + err);
                } else {
                    // Call the getPredictions() callback with the modified data object
                    callback(data.body.playlists.items);
                }
            });
        }, function(err) {
